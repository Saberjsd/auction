<template>
  <div class="wrap"  v-loading="loadingmask"
    ...
    <GroundStep1 ref="getOwner" v-if="!$store.getters.areaIn && $store.state.groundStep==0 && $store.state.sidebars != '04'" @goArea="render_area" @refreshHot="refreshAll()" :state="$store.state.groundStep" :landid="$store.state.currentGroundID"></GroundStep1>
    <GroundStep2 ref="currentID" v-if="!$store.getters.areaIn && $store.state.groundStep==1 && $store.state.sidebars != '04'" @goArea="render_area" @refreshHot="refreshAll()" :state="$store.state.groundStep" :landid="$store.state.currentGroundID"></GroundStep2>
  ...
  methods: {
        ...
        for (let i = 0; i < vthis.landList.length; i++) {
          ...
          if (vthis.landList[i].state == 1) {
            ...
            var label = new BMap.Label(
              parseInt(vthis.landList[i].topPrice / 1000) + "k",
              opts
            ); // 创建文本标注对象
            label.setStyle({
              color: "#ffffff",
    ...
    render_land(res,sidebar) {
          var vthis = this;
            ...
            res.data.data = res.data.data.map((item) => {  //修改数据结构
            ...
            this.totalIndex = this.$store.state.landList.length; //总的地块数
            this.index = 1                                      //当前的地块索引
            this.get_land_detail(res.data.data[0].id);
            this.$store.dispatch("setData", {        //设置当前地块ID
              state: "currentGroundID", 
              data: res.data.data[0].id
            });
            this.chageGroundStep(res.data.data[0].state);   //根据状态切换组件
            if (this.$store.state.landList) {
              var map = this.map;
              map.clearOverlays();
              map.setMinZoom(18)
              map.setMaxZoom(19)
              ...
              // //默认地块标注
              var markerP = this.$store.state.landList[0].centerPoint.split(",");
              var cp = new BMap.Point( parseFloat(markerP[0]), parseFloat(markerP[1]) );
              var marker = new BMap.Marker(cp); // 创建标注;
              map.addOverlay(marker);
              marker.setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画
              ...
              var PayloadList = [];
              var labelList=[];
              for (let i = 0; i < this.$store.state.landList.length; i++) {
                var item = this.$store.state.landList[i];
                ...
                var polygon = new BMap.Polygon( pointS,this.ployStyle[item.state]);
                polygon.pid = item.id;
                ...
                polygon.addEventListener("click", function(e){
                  ...
                  //创建小红点标注
                  // var land = vthis.$store.state.landList.filter(function (item) {
                  //   return item.id == this.pid;
                  // });
                  var lan = vthis.$store.currentGround.centerPoint.split(",");
                  // console.log(lan)
      ...
      this.$http
        ...
        .then(function(res) {
          ...
          if (res.data.code == 1) {
            ...
            vthis.totalIndex = vthis.$store.state.landList.length;
            vthis.index = 1
            // debugger;
      ...
      this.$http
        ...
        .then(function(res) {
          if (res.data.code == 1) {
            ...
            vthis.totalIndex = res.data.data.length;
            vthis.get_land_detail(res.data.data[0].land.id);
            vthis.$store.dispatch("setData", {
              state: "currentGroundID",
              data: res.data.data[0].land.id
          ...
          if (res.data.code == 1) {
            ...
            if (vthis.$store.state.landList) {
              ...
              for (let i = 0; i < vthis.$store.state.landList.length; i++) {
                ...
                // debugger;
                //添加文本标注 ------------------------
                if (vthis.landList[i].startPrice && !vthis.landList[i].topPrice) {
                  var piontCen = vthis.landList[i].centerPoint.split(",");
                  var piontCenter = new BMap.Point(
                    parseFloat(piontCen[0]),
                    parseFloat(piontCen[1])
                  ...
                  var label = new BMap.Label(
                    parseInt(vthis.landList[i].startPrice / 1000) + "k",
                    opts
                  ); // 创建文本标注对象
                  label.setStyle({
                    color: "#ffffff",
                ...
                polygon.addEventListener("click", function(e) {
                  ...
                  //创建小红点标注
                  if (vthis.marker) {
                    map.removeOverlay(vthis.marker);
                  }
                  // if(this.status == 1){
                  //   vthis.$store.state.currentGroundID = '0xWX4SDFSD1LKH';
                  // }else{
    ...
    refreshAll(side){
      this.chage_sidebar(side);
    },
    refresh_hot_land(callback) {
      // debugger;
      var vthis = this;
      this.$http
        .get("/auction/api/land/getByMark?isMark=2" ,{timeOut:1000})
        .then(function(res) {
          if (res.data.code == 1) {
            // debugger;
            //vthis.$store.dispatch('setData',{state:'sidebars',data:"05"})
